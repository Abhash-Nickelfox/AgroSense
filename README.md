...
npm run dev            # runs Express (port 5000) and Vite (port 5173) together
```

Open http://localhost:5173 — API calls to `/api/*` are proxied to the Express server.
...
- Section imagery uses the same hosted URLs from the Stitch export (Google-hosted AI renders + Unsplash placeholders); swap these for your own final assets when ready.
